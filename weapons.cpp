...
//monsters
int32_t Weapons::getMaxMeleeDamage(int32_t attackSkill, int32_t attackValue, int32_t strength, int32_t dexterity)
{
	return static_cast<int32_t>(std::ceil(((attackSkill + strength + dexterity) * (attackValue * 0.05)) + (attackValue * 0.5)));
}

//players
int32_t Weapons::getMaxWeaponDamage(uint32_t level, int32_t attackSkill, int32_t attackValue, float attackFactor, int32_t strength, int32_t dexterity)
{
	return static_cast<int32_t>(std::round(((level + strength + dexterity) / 5) + (((((attackSkill / 4.) + 1) * (attackValue / 3.)) * 1.03) / attackFactor)));
}
...
int32_t WeaponDistance::getWeaponDamage(const Player* player, const Creature* target, const Item* item, bool maxDamage /*= false*/) const
{
	...
	int32_t attackSkill = player->getSkillLevel(SKILL_DISTANCE);
	float attackFactor = player->getAttackFactor();
	if (item->getWeaponType() == WEAPON_WAND) {
		attackSkill += player->getInteligence() / 2;
	}

	int32_t maxValue = static_cast<int32_t>(Weapons::getMaxWeaponDamage(player->getLevel() + (player->getStrength() / 5), attackSkill, attackValue, attackFactor, player->getStrength(), player->getInteligence()) * player->getVocation()->distDamageMultiplier);
	if (maxDamage) {
		return -maxValue;
	}
